...
import { MapPin, Truck, Activity, Package } from "lucide-react";
import { cn } from "@/lib/utils";
import { OngoingOrderResponse } from "@/api/interfaces/suppliers";
import { Link } from "@/i18n/navigation";
import { useTranslations } from "next-intl";

interface OrderCardProps {
  order: OngoingOrderResponse;
  showStatus?: boolean;
}
...
export function OngoingOrderCard({ order, showStatus = true }: OrderCardProps) {
  const t = useTranslations("supplier.ongoingOrder");
        ...
        <div className="mt-4 space-y-3">
          <div className="flex items-center gap-x-2">
            ...
            <div className="flex space-x-1">
              <p className="text-sm font-medium">{t("serviceType")}:</p>
              <p className="text-sm font-bold text-gray-600">
                {tCommon(`quotationTypes.${order.quotation_type}`)}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-x-2">
            ...
            <div className="flex space-x-1">
              ...
              <p className="text-sm text-gray-600">{order.from_city}</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-x-2">
            ...
            <div className="flex space-x-1">
              ...
              <p className="text-sm text-gray-600">{order.to_city}</p>
            </div>
          </div>
