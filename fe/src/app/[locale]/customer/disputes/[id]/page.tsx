"use client";

import { useDisputeDetails } from "@/hooks/customer/useDisputeDetails";
import { formatDate } from "date-fns";
import { ArrowLeft, Box, FileText } from "lucide-react";
import { useRouter } from "@/i18n/navigation";
import { ImageIcon } from "lucide-react";
import { FullPageLoader } from "@/components/ui/loading/FullPageLoader";
import { capitalizeWords } from "@/lib/capitalizeWords";
import { cn } from "@/lib/utils";
import { Button } from "@/components/ui/button";
import { Link } from "@/i18n/navigation";
import { baseUrl } from "@/api";
import { useLocale, useTranslations } from "next-intl";
import { useParams } from "next/navigation";
import { formatDateLocale } from "@/lib/formatDateLocale";

const DisputeDetails = () => {
  const router = useRouter();
  const { id } = useParams<{ id: string }>();
  const { data, isLoading } = useDisputeDetails(id);
  const dispute = data?.data;
  const t = useTranslations("customers.disputeDetails");
  const locale = useLocale();

  // const getStatusColor = (status: string) => {
  //   switch (status.toLowerCase()) {
  //     case "resolved":
  //       return "bg-green-500 text-white";
  //     case "pending":
  //       return "bg-yellow-500 text-white";
  //     case "under_review":
  //       return "bg-blue-500 text-white";
  //     case "rejected":
  //       return "bg-red-500 text-white";
  //     default:
  //       return "bg-gray-500 text-white";
  //   }
  // };

  if (isLoading) {
    return <FullPageLoader />;
  }

  return (
    <div className="flex flex-col min-h-screen bg-gray-50">
      {/* Header */}
      <div className="bg-[#EC1B25] sticky top-0 z-50 backdrop-blur-sm md:rounded-[10px] rounded-b-[24px] p-2 md:p-4 mb-6 text-white">
        <div className="flex items-center justify-between mb-4 md:mb-6">
          <button
            onClick={() => router.back()}
            className="p-2 hover:bg-gray-100 rounded-full transition-colors"
          >
            <ArrowLeft className="w-6 h-6" />
          </button>
          <div className="w-10" /> {/* Spacer for centering */}
        </div>

        {/* Dispute Info */}
        <div className="flex md:flex-row flex-col md:items-center mb-6 md:mb-8">
          <div className="flex gap-x-4 items-start justify-start">
            <div className="bg-white flex justify-center items-center rounded-full w-12 h-12">
              <Box size={24} color="red" />
            </div>
            <div>
              <h2 className="text-xl font-semibold">
                {t("disputeId")}: {dispute?.dispute_id}
              </h2>
              <p className="font-bold text-[32px]">
                {capitalizeWords(dispute?.status)}
              </p>
            </div>
          </div>
          <span
            className={cn(
              "ml-auto px-4 py-1 rounded-full",
              dispute?.status === "resolved"
                ? "bg-green-100 text-green-600"
                : "bg-yellow-100 text-yellow-600"
            )}
          >
            {dispute?.status === "resolved" ? t("completed") : t("inProgress")}
          </span>
        </div>
      </div>

      {/* Content */}
      <div className="p-4 space-y-6">
        {/* Subject */}
        <div>
          <h2 className="text-gray-600">{t("subject")}:</h2>
          <p className="font-medium">
            {t("disputeRaised")} #{dispute?.dispute_id}
          </p>
        </div>

        {/* Dates */}
        <div className="space-y-4">
          <div>
            <h2 className="text-gray-600">{t("dateRaised")}:</h2>
            <p className="font-medium">
              {dispute?.created_at &&
                formatDateLocale(dispute?.created_at, locale)}
            </p>
          </div>
          <div>
            <h2 className="text-gray-600">{t("orderId")}:</h2>
            <p className="font-medium">#{dispute?.dispute_id}</p>
          </div>
          <div>
            <h2 className="text-gray-600">{t("orderDate")}:</h2>
            <p className="font-medium">
              {dispute?.created_at &&
                formatDateLocale(dispute?.created_at, locale)}
            </p>
          </div>
        </div>

        {/* Dispute Summary */}
        <div>
          <h2 className="text-lg font-semibold mb-4">{t("disputeSummary")}</h2>
          <div className="space-y-4">
            <div>
              <h3 className="text-gray-600">{t("complaintDescription")}:</h3>
              <p className="mt-1">{dispute?.request_details}</p>
            </div>
            <div>
              <h3 className="text-gray-600">{t("requestedAction")}</h3>
              <p className="mt-1">{dispute?.reason}</p>
            </div>
          </div>
        </div>

        {/* Resolution Outcome - Only show if resolved */}
        {dispute?.status === "resolved" && (
          <div>
            <h2 className="text-lg font-semibold mb-4">
              {t("resolutionOutcome")}
            </h2>
            <div className="space-y-4">
              <div>
                <h3 className="text-gray-600">{t("decision")}</h3>
                <p className="mt-1">{t("partialRefundApproved")}</p>
              </div>
              <div>
                <h3 className="text-gray-600">{t("refundedAmount")}</h3>
                <p className="mt-1">SEK {dispute?.transaction_amount}</p>
              </div>
              <div>
                <h3 className="text-gray-600">{t("comments")}</h3>
                <p className="mt-1">{dispute?.request_details}</p>
              </div>
            </div>
          </div>
        )}

        {/* Evidence */}
        <div>
          <h2 className="text-lg font-semibold mb-4">{t("evidenceSummary")}</h2>
          <div className="space-y-4">
            <div>
              <h3 className="text-gray-600 mb-2">{t("evidence")}</h3>
              <div className="flex flex-wrap gap-2">
                {dispute?.images?.map((image, index) => (
                  <div
                    key={index}
                    className="flex items-center gap-2 bg-gray-100 p-2 rounded"
                  >
                    {image.endsWith(".pdf") ? (
                      <button
                        key={index}
                        onClick={() =>
                          window.open(
                            baseUrl.slice(0, baseUrl.length - 1) + image,
                            "_blank"
                          )
                        }
                        className="px-2 py-1 items-center gap-x-2 flex bg-[#C4C4C4] rounded-[10px]"
                      >
                        <FileText color="#2E7D32" size={16} />
                        <div className="text-subtitle dark:text-white text-[12px] font-medium">
                          ..{image.slice(-8)}
                        </div>
                      </button>
                    ) : (
                      <button
                        key={index}
                        onClick={() =>
                          window.open(
                            baseUrl.slice(0, baseUrl.length - 1) + image,
                            "_blank"
                          )
                        }
                        className="px-2 py-1 items-center gap-x-2 flex bg-[#C4C4C4] rounded-[10px]"
                      >
                        <ImageIcon className="h-5 w-5" />
                        <div className="text-subtitle dark:text-white text-[12px] font-medium">
                          ..{image.slice(-8)}
                        </div>
                      </button>
                    )}
                    <span>
                      evidence_{index + 1}
                      {image.split(".").pop()}
                    </span>
                  </div>
                ))}
              </div>
            </div>
            {/* <div>
              <h3 className="text-gray-600 mb-2">Mover Evidence</h3>
              <div className="flex flex-wrap gap-2">
                {dispute?.images?.map((image, index) => (
                  <div
                    key={index}
                    className="flex items-center gap-2 bg-gray-100 p-2 rounded"
                  >
                    {image.endsWith(".pdf") ? (
                      <FileIcon className="h-5 w-5" />
                    ) : (
                      <ImageIcon className="h-5 w-5" />
                    )}
                    <span>
                      evidence_{index + 1}
                      {image.split(".").pop()}
                    </span>
                  </div>
                ))}
              </div>
            </div> */}
          </div>
        </div>
      </div>
      {dispute?.status !== "resolved" && (
        <Button className="mb-8 mx-12 md:mx-24 font-semibold">
          <Link href={`/customer/disputes/chat/${id}`}>{t("message")}</Link>
        </Button>
      )}
    </div>
  );
};

export default DisputeDetails;
