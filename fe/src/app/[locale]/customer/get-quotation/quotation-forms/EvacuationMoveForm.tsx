...
import { EvacuationMove, createEvacuationMoveSchema } from "../i18n-schema";
...
interface EvacuationMoveFormProps {
  onSubmit: (data: EvacuationMove) => void;
  mutation: UseMutationResult<unknown, unknown, unknown>;
  formStage: number;
  handleNext: () => void;
}

const EvacuationMoveDetailsForm = ({ form, handleNext }: any) => {
  const t = useTranslations("quotation.evacuationMove");
  ...
  const services = [
    { id: 1, value: "transport", label: t("services.transport") },
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <FormField
        ...
        render={({ field }) => (
          <FormItem>
            <FormLabel>{t("propertySize")} (m²)</FormLabel>
            <FormControl>
              <Input
                type="number"
                value={field.value}
...
export const EvacuationMoveForm: React.FC<EvacuationMoveFormProps> = ({
  onSubmit,
  mutation,
  formStage,
  handleNext,
  ...
  const form = useForm<EvacuationMove>({
    resolver: zodResolver(createEvacuationMoveSchema(t)),
    defaultValues: {
      ...
      type_of_service: "Evacuation Move",
      service_type: "Evacuation Move",
      move_type: "apartment",
      from_rok: 1,
      to_rok: 1,
      from_floor: "",
  ...
  return (
    <FormWrapper form={form} onSubmit={onSubmit}>
      {formStage === 1 ? (
        ...
        <EvacuationMoveDetailsForm form={form} handleNext={handleNext} />
      ) : formStage === 3 ? (
