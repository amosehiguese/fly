                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-baseline">
                    <h3 className="font-medium truncate text-gray-800 dark:text-gray-200">
                      {`${msg.other_party_name === "admin" || msg.other_party_name === "supplier" || msg.other_party_name === "customer" ? tCommon(`chats.recipient.${msg.other_party_name}`) : msg.other_party_name} (${tCommon.raw("chats.reasonList").find((reason: { id: string; name: string }) => reason.id === msg.reason)?.name})`}
                    </h3>
                    <span className="text-xs text-gray-500 dark:text-gray-400 flex-shrink-0 ml-2">
                      {formatDateLocale(msg.last_message_time, locale)}
                    </span>
                  </div>
