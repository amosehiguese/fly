...
import { useRegisterDriver } from "@/hooks/supplier/useDrivers";
...
export default function SupplierRegisterDriverPage() {
  ...
  const submitRegistration = () => {
    // Comment out random password generator
    // // Generate a random password for the driver
    // // In a real scenario, you might want to send an email to the driver with a password reset link
    ...
    const formData = {
      ...
      phoneNumber: phone,
      email,
      password, // Using password entered in the form
      licenseNumber,
      ...
      // Could add a supplier_id field here if the API requires it
    };

    registerDriver.mutate(formData, {
      onSuccess: () => {
        toast.success(t("registrationSuccess"));
        router.push("/supplier/drivers"); // Redirect to drivers list instead of login
      },
  ...
  const handleBack = () => {
    if (currentStep > RegistrationStep.PERSONAL_INFO) {
      ...
      router.push("/supplier/drivers"); // Back to supplier drivers page
    }
  };
  ...
  return (
    <div className="min-h-screen p-4 flex flex-col bg-background max-w-md mx-auto md:max-w-lg lg:max-w-xl">
      {/* Header */}
      <div className="mb-4">
